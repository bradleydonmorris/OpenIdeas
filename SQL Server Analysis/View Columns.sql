		...
		SELECT
			...
			(
				...
				CASE
					...
					(
						...
						(
							CASE
								WHEN CONVERT([int], COLUMNPROPERTY([views].[object_id], [columns].[name], 'CharMaxLen'), 0) = (-1)
									THEN '(MAX)'
								ELSE
								(
									'('
									+ CONVERT([varchar](11), CONVERT([int], COLUMNPROPERTY([views].[object_id], [columns].[name], 'CharMaxLen'), 0), 0)
									+ ')'
								)
							END
						)
			...
			FROM [sys].[views]
				INNER JOIN [sys].[schemas] AS [schemas_Object]
					ON [views].[schema_id] = [schemas_Object].[schema_id]
				...
				LEFT OUTER JOIN [sys].[computed_columns]
					ON
						[views].[object_id] = [computed_columns].[object_id]
						AND [columns].[column_id] = [computed_columns].[column_id]
			WHERE
				[schemas_Object].[name] = N'AcquireSellExcellence'
	) AS [Column]
	ORDER BY
		[Column].[SchemaName],
		[Column].[ViewName],
