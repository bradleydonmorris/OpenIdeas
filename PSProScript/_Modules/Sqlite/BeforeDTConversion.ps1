...
Add-Member `
    ...
    -Value {
        ...
        Try
        {
            ...
            Else
            {
                If (![String]::IsNullOrEmpty($FieldConversion))
                {
                    $ReturnValue = $Global:Session.Sqlite.ConvertFromDBValue($ReturnValue, $FieldConversion);
                }
            }
        }
        Finally
...
Add-Member `
    ...
    -Value {
        ...
        Try
        {
            $Connection = [Data.Sqlite.SqliteConnection]::new($Global:Session.Sqlite.GetConnectionString($ConnectionName));
            $Connection.Open();
            $Command = [Data.Sqlite.SqliteCommand]::new([String]::Format("REINDEX ``{0}``", $TableName), $Connection);
            $Command.CommandType = [Data.CommandType]::Text;
            [void] $Command.ExecuteNonQuery();
        }
        Finally
